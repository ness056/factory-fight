...
return {
    {
        f = function (event)    -- ness - perma chart some chunk to forces
            ...
            local x1Blue = -x1 - (Config.generation.playerBoxMaxWidth + Config.generation.bordersWidth) * (math.floor(global["blue" .. "BoxN"] / Config.generation.playerNBoxPerLine) + 1)
            local x1Red = x1 + (Config.generation.playerBoxMaxWidth + Config.generation.bordersWidth) * (math.floor(global["red" .. "BoxN"] / Config.generation.playerNBoxPerLine) + 1)
